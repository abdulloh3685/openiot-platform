...
class Relay { public: foundation::ErrorCode begin(std::uint8_t pin); foundation::ErrorCode begin(std::uint8_t pin, bool active_low); foundation::ErrorCode set(bool on); bool state() const { return on_; } private: std::uint8_t pin_{0}; bool ready_{false}; bool on_{false}; bool active_low_{false}; };
} // namespace openiot::drivers
