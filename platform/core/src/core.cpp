...
void EventBus::end() { initialized_ = false; for (auto& s : subscriptions_) { s.type = EventType::BootCompleted; s.handler = nullptr; s.used = false; } }
foundation::ErrorCode EventBus::subscribe(EventType type, EventHandler handler) {
    ...
    for (auto& s : subscriptions_) if (!s.used) {
        s.type = type;
        s.handler = handler;
        s.used = true;
...
foundation::ErrorCode EventBus::unsubscribe(EventType type, EventHandler handler) {
    if (!initialized_) return foundation::ErrorCode::NotInitialized;
    for (auto& s : subscriptions_) if (s.used && s.type == type && s.handler == handler) {
...
foundation::Result<std::uint8_t> Scheduler::registerTask(Task task, void* context, std::uint32_t period_ms) {
    ...
    for (std::uint8_t i = 0; i < tasks_.size(); ++i) if (!tasks_[i].used) {
        tasks_[i].task = task;
...
foundation::ErrorCode Config::set(const char* key, const char* value) {
    ...
    for (auto& e : entries_) if (e.used && std::strcmp(e.key, key) == 0) { std::strncpy(e.value, value, sizeof(e.value)-1); e.value[sizeof(e.value)-1] = '\0'; return foundation::ErrorCode::Ok; }
